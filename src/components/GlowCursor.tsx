...
void main() {
  ...
  vec3 finalColor = mix(additiveColor, normalColor * normalAlpha, uNormalBlend);
  gl_FragColor = vec4(finalColor, mix(alpha, normalAlpha, uNormalBlend));
}
`;
...
const checkWebGLSupport = (canvas: HTMLCanvasElement): boolean => {
  try {
...
export const GlowCursor = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn('GlowCursor WebGL context initialization skipped:', err);
      canvas.style.display = 'none';
      return;
    }
    ...
    const render = (now: number) => {
      ...
      // When fully faded and idle, pause render loop to free GPU during scroll
